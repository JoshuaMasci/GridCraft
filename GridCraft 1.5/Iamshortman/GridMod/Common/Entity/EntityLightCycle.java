...
public class EntityLightCycle extends TronEntity
{
	...
	{
		return "/mods/GridCraft/textures/lightCycle/LightCycleGlow.png";
	}

	@Override
	public String getBaseTexture()
	{
		return "/mods/GridCraft/textures/lightCycle/LightCycleBase.png";
	}
	...
	public void onUpdateWithPilotPlayerInput()
	{
		...
		if (player != null)
		{
			if (Minecraft.getMinecraft().gameSettings.keyBindForward.pressed && (this.rotationYawSpeed > 10 || this.rotationYawSpeed < -10) && this.onGround && !this.Gridlock)
			{
				this.rotationRoll = -(this.rotationYawSpeed / 5F);
			}
			else
			{
				if (this.rotationRoll < -1)
				{
					this.rotationRoll++;
				}
				else if (this.rotationRoll > 1)
				{
					this.rotationRoll--;
				}
				else
			...
			if (Minecraft.getMinecraft().gameSettings.keyBindForward.pressed || Minecraft.getMinecraft().gameSettings.keyBindBack.pressed)
			{
				if(!this.Gridlock)
				{
					if (this.minecraft.currentScreen == null && minecraft.gameSettings.thirdPersonView != 2 && this.turnWait == 0 && this.onGround)
					{
	...
	{
		this.Model = new ModelLightCycle();
	}
